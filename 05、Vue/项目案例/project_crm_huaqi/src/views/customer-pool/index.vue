  ...
  <div class="customer-pool-box">
        ...
        <el-form-item label="意向程度">
         ...
         <el-form-item>
          <el-dropdown
            ...
            type="primary"
            size="medium"
            @command="customerSelectClick"
          >
            {{ customerSelectValue == 0 ? "公共池" : "私有池" }}
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item command="0">公共池</el-dropdown-item>
              <el-dropdown-item command="1">私有池</el-dropdown-item>
            </el-dropdown-menu>
          </el-dropdown>
        </el-form-item>
         <el-form-item>
            ...
            <el-button type="danger" size="medium" :loading="dangerLoading" @click="TopDelectFnc('All')">删除多条</el-button>
          </el-form-item>
      </el-form>
    </div>
    <!-- 主体栏 -->
    <div class="customer-poll-Contaner">
      ...
      <div class="pageComponent">
        ...
        <div>
          <el-button type="primary" size="medium" @click="importFile" v-show="customerSelectValue == 0" :loading="importLoading">导入</el-button>
          <el-button type="warning" size="medium" @click="exportExcel" :loading="exportLoading">导出</el-button>
        </div>
      </div>
    </div>
    <!-- 详情弹窗组件 -->
...
import { CustomerPool ,importMultipleData} from '@/api/customerpool'
export default {
  name: "customer-pool",
  components: {
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      ...
      this.queryCustomerPool()
      console.log( this.queryCustomerPool())
    },
    ...
    /**
     * @description: 公有/私有池选择事件
     * @param {*}
     * @return {*}
     */
    customerSelectClick(val) {
      console.log(val);
      if(val == this.customerSelectValue){
        return false;
      }
      this.loading = true;
      this.customerSelectValue = val;
      this.queryCustomerPool();
    },
    ...
    importFile() {
      ...
      Excel.importExcel((data, dataRef) => {
        this.loading = true;
        console.log(data);
        let objKeyArr = data[0];
        let newArr = [];
        for (let index = 1; index < data.length; index++) {
          const obj = {};
          data[index].forEach((item, index) => {
            obj[objKeyArr[index]] = item;
          });
          newArr.push(obj);
        }
        importMultipleData({formData:newArr,user_name:this.name}).then(response => {
          console.log(response)
          this.queryCustomerPool();
          this.loading = false;
